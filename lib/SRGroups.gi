...
InstallGlobalFunction(FormatSRFile, function(deg,lev)
	...
	if IsExistingFile(fNew) then
		...
		if not reEntry then
			...
			for k in [1..numGroups] do
				gens[k]:=GeneratorsOfGroup(Image(prBelow,Group(projBelow[k][1][1]))); # Generators of the projected image of the first group from projBelow[k].
			od;
		...
		while k<=numGroups do
			...
			gensAboveTemp:=GeneratorsOfGroup(Image(pr,Group(gens[k]))); # Generators of the projected image of the group on level lev generated by gens[k].
			...
			if Group(gensAboveTemp)=Group(currentGens) then
				# 3.2.2.2.1. If the generated group is not unique (always the case when k=1 since currentGens is initialised as trivial), currentGens stays the same and the number of identical groups in numProj[numGroupsAbove] is increased by 1.
			...
			PrintTo(fVariables,StringVariables(numGroups,k,gens,numProj,numGroupsAbove,gensAbove)); # Save-point
	...
	while m<=numGroups do
		...
		AppendTo(fSingleGroup, "\n\t\t", groupInfo[m][2], ",");
		AppendTo(fSingleGroup, "\n\t\t", groupInfo[m][3], ",");
		# 4.2.1. Index 4 must reflect the known groups each group on level lev extends to (using projBelow[m]).
		groupInfo[m][4]:=[];
		...
		# 4.5. Save this point.
		if IsExistingFile(fNew) then
			PrintTo(fVariables,StringVariables(numGroups,gens,numProj,numGroupsAbove,aboveCount,m)); # Save-point (case 1)
		else
			PrintTo(fVariables,StringVariables(numGroups,k,gens,numProj,numGroupsAbove,gensAbove,aboveCount,m)); # Save-point (case 2)
		fi;
	...
	# 2. Case where the input argument is in [2,31].
		...
		# 2.5. Level>1 case.
			...
			if not projectionProtocol then
				# Extend each group on level lev-1 to all conjugacy class representatives and store their generators in the file "temp_deg_lev.grp".
				groupGens:=[];
